  ...
  return (
    <main className="min-h-screen bg-[#f7f7f4] text-neutral-950 lg:grid lg:grid-cols-[17rem_1fr]">
      <aside className="border-b border-neutral-200 bg-white px-5 py-5 lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:justify-between lg:border-b-0 lg:border-r lg:px-6 lg:py-7">
        <div>
          <div id="inicio">
            ...
            <h1 className="mt-2 font-[var(--font-display)] text-3xl font-normal tracking-[-0.04em] lg:text-4xl">
              Conteúdo
            </h1>
          ...
          >
            {navItems.map((item) => (
              <Link
                className="inline-flex min-h-11 shrink-0 items-center border border-neutral-200 px-4 text-sm uppercase tracking-[0.14em] text-neutral-700 hover:border-neutral-950 hover:text-neutral-950 focus:outline focus:outline-2 focus:outline-offset-4 focus:outline-neutral-950 lg:border-transparent lg:px-0"
                href={item.href}
                key={item.href}
              >
                {item.label}
        ...
        <div className="mt-6 flex flex-wrap gap-3 lg:grid">
            ...
            <form action={logoutAdminAction}>
              <button
                className="min-h-11 w-full border border-neutral-950 px-4 text-sm uppercase tracking-[0.16em] hover:bg-neutral-950 hover:text-white focus:outline focus:outline-2 focus:outline-offset-4 focus:outline-neutral-950"
                type="submit"
              >
                Sair
              </button>
      ...
      <div className="px-5 py-6 md:px-8 md:py-10 xl:px-12">
        {status ? (
          <p
            className="mb-6 border border-neutral-950 bg-white px-4 py-3 text-sm"
            role="status"
          >
            {status}
          </p>
        ...
        {error ? (
          <p
            className="mb-6 border border-neutral-950 bg-neutral-950 px-4 py-3 text-sm text-white"
            role="alert"
          >
            {error}
          </p>
