...
import type { MediaAsset, MediaUsageScope } from "@/lib/db/schema";
...
function getDisplayNameFromAsset(asset: MediaAsset) {
  return asset.altText.replace(/ - (rolagem otimizado|normal com áudio)$/, "");
}

function getLibraryItems(mediaAssets: MediaAsset[]) {
...
export function MediaUploadField({
  ...
  async function uploadFile(file: File) {
    ...
    if (file.type.startsWith("video/")) {
      ...
      const videoResponse = await fetch("/api/uploads/video", {
        body: videoFormData,
        method: "POST",
      });
    ...
    if (!signResponse.ok || !signedUpload.uploadUrl || !signedUpload.storageKey) {
